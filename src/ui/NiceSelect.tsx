...
const NiceSelect: FC<NiceSelectProps> = ({ options, defaultCurrent, placeholder, className, onChange, name, value }) => {
  ...
  useEffect(() => {
      const newCurrent = options.find((v) => v.value === value);
      if (newCurrent) {
        currentHandler(newCurrent);
      }
  }, [value]);
  ...
  return (
    <div className={`nice-select form-select-lg ${className || ''} ${open ? 'open' : ''}`} role="button" tabIndex={0} onClick={() => setOpen((prev) => !prev)} onKeyDown={(e) => e} ref={ref}>
      ...
      <ul className="list" role="menubar" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
        {options?.map((item, i) => (
          <li key={i} data-value={item.value} className={`option ${item.value === current?.value ? 'selected focus' : ''}`} style={{ fontSize: '14px' }} role="menuitem" onClick={() => currentHandler(item)} onKeyDown={(e) => e}>
            {item.text}
          </li>
        ))}
      </ul>
